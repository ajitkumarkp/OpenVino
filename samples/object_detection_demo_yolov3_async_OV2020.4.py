...
def main():
    ...
    exec_nets[Modes.USER_SPECIFIED] = ie.load_network(network=net, device_name=args.device,config=config_user_specified,num_requests=args.num_infer_requests)
    
    #@jit for multi-device plugin
    # exec_nets[Modes.USER_SPECIFIED] = ie.load_network(network=net, device_name="MULTI",config={},num_requests=args.num_infer_requests)

    exec_nets[Modes.MIN_LATENCY] = ie.load_network(network=net, device_name=args.device.split(":")[-1].split(",")[0], config=config_min_latency, num_requests=1)
    ...
    while (cap.isOpened() \
        ...
        if next_frame_id_to_show in completed_request_results:
                ...
                if args.raw_output_message:
                    log.info(
                        "{:^9} | {:10f} | {:4} | {:4} | {:4} | {:4} | {} ".format(det_label, obj['confidence'],
                                                                                  ...
                                                                                  color))
                if (det_label == 'bottle'):
                    (xmin_bot, ymin_bot) = (obj['xmin'], obj['ymin'])
                
                # if (det_label == 'bowl'):
                #     (xmin_bow, ymin_bow) = (obj['xmin'], obj['ymin'])

                if (det_label == 'person'):
                    (xmax_per, ymax_per) = (obj['xmax'], obj['ymax'])
                    if (xmin_bot < xmax_per and ymin_bot < ymax_per ):
                        bot_in_basket = True
                    # if (xmin_bow < xmax_per and ymin_bow < ymax_per ):
                    #     bow_in_basket = True
                
                if (bot_in_basket):
                    color = (0, 0, 255)
                    cv2.putText(frame, "!!Person picked an Object!!", (obj['xmin']+5, obj['ymin']+20), cv2.FONT_HERSHEY_COMPLEX, 1, color, 1)
                
                # if (bow_in_basket):
                #     color = (0, 0, 255)
                #     cv2.putText(frame, "!!Person picked an Object!!", (obj['xmin']+5, obj['ymin']+20), cv2.FONT_HERSHEY_COMPLEX, 1, color, 1)
                
                if (det_label == 'person' or det_label == 'bottle' or det_label == 'bowl' or det_label == 'chair'):
                    ...
                    if bot_in_basket:
                        put_highlighted_text(frame, "1x Bottle..... $4.95", (15, 110), cv2.FONT_HERSHEY_COMPLEX, 0.75, (255,0,0), 2)

                    if bow_in_basket:
                        put_highlighted_text(frame, "1x Bowl....... $10.95", (15, 140), cv2.FONT_HERSHEY_COMPLEX, 0.75, (255,0,0), 2)

                put_highlighted_text(frame, "##### CUSTOMER BASKET #####", (15, 80), cv2.FONT_HERSHEY_COMPLEX, 0.75, (255,0,0), 2)
            ...
            if mode_info[mode.current].frames_count != 0:
                ...
                if len(objects) and args.raw_output_message: #@jit
                    log.info(fps_message)
                    log.info(latency_message)
            ...
            if not args.no_show:
                # frame = cv2.resize(frame, (1900, 1000)) # @jit
